...
function copiarIP() {
    chrome.storage.sync.get(['enable_copyip'], (response) => {
        if (!response.enable_copyip) {
            ...
            if ($('.game-modal-ready-button').is(':visible')) {
                chrome.storage.sync.get([`enable_notification`], (response) => {
                    if (!response.enable_notification) {
                        console.log('Notificações desativadas.')
                    } else {
...
function joinServer() {
    chrome.storage.sync.get([`enable_joinServer`], (response) => {
        if (!response.enable_joinServer) {
            return
        } else {
            if ($('.game-modal-ready-button').is(':visible')) {
                if ((document.querySelector('.game-modal-ready-button').href).includes('connect')) {
                    window.location.href = document.querySelector('.game-modal-ready-button').href
                }
            } else {
                joinServer();
            }

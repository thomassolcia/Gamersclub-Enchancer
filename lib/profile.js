        ...
        if (!response.enable_porcentagem) {
            ...
            chrome.storage.sync.get([`enable_notification`], (response) => {
                if (!response.enable_notification) {
                    ...
                    chrome.storage.sync.get([`notification_porcentagem`], (response) => {
                        if (!response.notification_porcentagem) {
                            return
                        } else {
                            //FREE
                            $('.MainHeader.MainHeader--free').append(`<div id="notifyFixa"; class="col-sm-12"><div style="border-radius:5px; margin:5px; padding:5px; border: 1px solid #839800; background-color: rgba(7, 149, 66, 0.12156862745098039); box-shadow: 0px 0px 2px #259c08; color: white;"><i class="start-icon far fa-check-circle faa-tada animated"></i><strong class="font__weight-semibold"> PORCENTAGEM CALCULADA!</strong> Veja sua porcentagem de vitórias!</div></div>`);
                            setTimeout(() => {
                            ...
                            //PREMIUM AND PLUS
                            $('.MainHeader').append(`<div id="notifyFixaPP"; class="col-sm-12"><div style="border-radius:5px; margin:5px; padding:5px; border: 1px solid #839800; background-color: rgba(7, 149, 66, 0.12156862745098039); box-shadow: 0px 0px 2px #259c08; color: white;"><i class="start-icon far fa-check-circle faa-tada animated"></i><strong class="font__weight-semibold"> PORCENTAGEM CALCULADA!</strong> Veja sua porcentagem de vitórias!</div></div>`);
                            setTimeout(() => {
